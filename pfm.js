...
// Global variables
var map, usermark, markers = [], userLat, userLng,
...
//
	...
	$('#opinionstd').insertAfter($('form'));
	$('#opinionaverage').insertAfter($('form'));
	var opinionAverage = $('#opinionaverage');
	var opinionAverageVal = opinionAverage.text().indexOf('-');
	if(opinionAverage.text().indexOf('-') !== -1){
		opinionAverage.css('background-color', 'red');
	} else {
		opinionAverage.css('background-color', 'aqua');
	}
...
// first step is to mark the color division as white and to say "Querying"
	...
	var cycles = '(';
	$.each($("input[name='data_type']:checked"), function(){
		data_type_string += "," + this.value;
	});
	$.each($("input[name='cycles']:checked"), function(){
		cycles += this.value + ',';
	});
	if($("input[name='cycles']:checked").length !== 0){
...
submitOpinion = function(){
	var color = $("select[name=redWhiteBlue]").val();
	var colorval;
	if (color== 'red')
		{colorval=-1;}
	else if (color=='white')
